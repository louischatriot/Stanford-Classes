...
public class NaiveBayes {
  ...
  public static double positiveReviews = 0.0;
  public static double negativeReviews = 0.0;

  public static HashMap<String, Double> positiveCounts = new HashMap<String, Double>();
  public static HashMap<String, Double> negativeCounts = new HashMap<String, Double>();

  public static double positiveTotal = 0.0;
  public static double negativeTotal = 0.0;

  public void addOneToWordCount(HashMap<String, Double> theMap, String word) {
    if (theMap.containsKey(word)) {
      theMap.put(word, theMap.get(word) + 1.0);
    } else {
      theMap.put(word, 1.0);
    }
  }
  ...
  public void addExample(String klass, List<String> words) {
    if (klass.equals("pos")) {
      this.positiveReviews += 1.0;
    } else {
      this.negativeReviews += 1.0;
    }


    for (String word : words) {
      if (klass.equals("pos")) {
        addOneToWordCount(this.positiveCounts, word);
      } else {
        addOneToWordCount(this.negativeCounts, word);
      }
    }
    ...
    //System.out.println(this.positiveReviews);
    //System.out.println(this.negativeReviews);
  ...
  public Double getProbability(HashMap<String, Double> theMap, String word, String type) {
    Double total = 0.0;

    if (type == "pos") {
      if (this.positiveTotal == 0) {
        for (Double d : this.positiveCounts.values()) {this.positiveTotal += d;}
      }

      total = this.positiveTotal;
    } else {
      if (this.negativeTotal == 0) {
        for (Double d : this.negativeCounts.values()) {this.negativeTotal += d;}
      }

      total = this.negativeTotal;
    }




    Double denominator = total + theMap.size() + 1;

    Double result = 0.0;
    if (theMap.containsKey(word)) {
      result = (theMap.get(word) + 1.0) / denominator;
    } else {
  ...
  public String classify(List<String> words) {
    Double posScore = Math.log(this.positiveReviews / (this.positiveReviews + this.negativeReviews));
    Double negScore = Math.log(this.negativeReviews / (this.positiveReviews + this.negativeReviews));

    for (String word : words) {
      posScore += Math.log(this.getProbability(this.positiveCounts, word, "pos"));
      negScore += Math.log(this.getProbability(this.negativeCounts, word, "neg"));
    }
  ...
  public void train(String trainPath) {
    ...
    //System.out.println("BLIK BLAK BLUK");
  ...
  public static void main(String[] args) {
    ...
    for(TrainSplit split: splits) {
      ...
      //classifier.addOneToWordCount(classifier.positiveCounts, "maison");
      //classifier.addOneToWordCount(classifier.positiveCounts, "chat");
      //classifier.addOneToWordCount(classifier.positiveCounts, "maison");
      //classifier.addOneToWordCount(classifier.positiveCounts, "maison");
      //System.out.println(classifier.positiveCounts);

      //System.out.println(classifier.positiveCounts.values());

      //Double total = 0.0;
      //for (Double d : classifier.positiveCounts.values()) {total += d;}
